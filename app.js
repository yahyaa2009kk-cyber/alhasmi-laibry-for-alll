...
function createPurchaseModal(){
    ...
    <div class="purchase-box" role="dialog" aria-modal="true">
      ...
      <div id="paymentArea" style="display:none">
        <div class="payment-box">
          ...
          <img id="paymentQr" class="payment-qr" src="${PAYMENT_QR_URL}" alt="QR الدفع - مكتبة الهاشمي">
  ...
  async function loadPaymentSettings(){
    ...
    if(error || !data){
      ...
      $('copyMaster').onclick=async()=>{try{await navigator.clipboard.writeText(MASTER_NUMBER);toast('تم نسخ رقم الماستر ✅')}catch{toast('انسخ الرقم يدوياً.')}};
      return;
    }
    ...
    $('copyMaster').onclick=async()=>{
      try{
        await navigator.clipboard.writeText(MASTER_NUMBER);
    ...
    $('openSuperQi').onclick=()=>{
      ...
      if(/Android/i.test(navigator.userAgent)){
        window.location.href='intent://#Intent;scheme=superqi;package=iq.qicard.qipay.prod;end';
        setTimeout(()=>window.open(playUrl,'_blank','noopener'),1400);
      }else{
