...
function createPurchaseModal(){
    ...
    <div class="purchase-box" role="dialog" aria-modal="true">
      ...
      <div id="paymentArea" style="display:none">
        ...
        <div class="payment-box">
          ...
          <div class="qr-note">
            افتح تطبيق سوبر كي وامسح رمز QR أعلاه لإتمام التحويل.
          </div>
  ...
  async function loadPaymentSettings(){
    ...
    const {data,error}=await db
      ...
      .select(
        'master_name,master_number,whatsapp_number,instructions'
      )
      .eq('id',1)
      .maybeSingle();
      ...
      try{
        ...
        result.innerHTML=`
          ...
          <div style="margin-top:8px">
            امسح QR عبر سوبر كي وحوّل المبلغ،
            ثم اكتب رقم الإيصال واضغط «تم التحويل».
          </div>
...
async function load(){
  ...
  try{
    ...
    const {data,error}=await db
      .from('content')
      .select(
        'id,title,type,stage,subject,price,file_path,branch,created_at'
      )
      .order(
        'created_at',
        {ascending:false}
    ...
    if(cards){
      ...
      cards.innerHTML=
        '<div class="empty">تعذر تحميل المحتوى حاليًا.</div>';
...
function render(){
  ...
  cards.innerHTML=
    rows.map(x=>{
      ...
      return `
        <article class="card user-item-card">
    ...
    '<div class="empty">لا يوجد محتوى مطابق حاليًا.</div>';

  document
    .querySelectorAll('.buy-content-btn')
    .forEach(btn=>{
  ...
  try{
    ...
    if(!window.supabase){
      ...
      if(statusEl){
        ...
        statusEl.textContent=
          'تعذر تشغيل الموقع: مكتبة Supabase غير محملة.';
    ...
    if(
      ...
      if(statusEl){
        ...
        statusEl.textContent=
          'تعذر تشغيل الموقع: إعدادات Supabase غير موجودة.';
